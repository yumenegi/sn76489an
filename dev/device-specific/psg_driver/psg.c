...
#define SN76489AN_REF_CLK 3500000
...
void PSG_Write(uint8_t payload) {
  ...
  // Note that D0 is MSB
  HAL_GPIO_WritePin(SN_D0_GPIO_PORT, SN_D0_PIN, (payload & 0x80) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D1_GPIO_PORT, SN_D1_PIN, (payload & 0x40) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D2_GPIO_PORT, SN_D2_PIN, (payload & 0x20) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D3_GPIO_PORT, SN_D3_PIN, (payload & 0x10) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D4_GPIO_PORT, SN_D4_PIN, (payload & 0x08) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D5_GPIO_PORT, SN_D5_PIN, (payload & 0x04) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D6_GPIO_PORT, SN_D6_PIN, (payload & 0x02) ? GPIO_PIN_SET : GPIO_PIN_RESET);
  HAL_GPIO_WritePin(SN_D7_GPIO_PORT, SN_D7_PIN, (payload & 0x01) ? GPIO_PIN_SET : GPIO_PIN_RESET);
...
void PSG_SetFrequency(uint8_t channel, uint16_t freq) {
  ...
  uint8_t payload = 
    SN76489AN_LATCH_BYTE | (channel << 5) | SN76489AN_FREQ_SEL 
...
void PSG_SetAttenuation(uint8_t channel, uint8_t attenuation) {
  // Construct payload and send latch byte
  uint8_t payload = 
    SN76489AN_LATCH_BYTE | (channel << 5) | SN76489AN_ATTEN_SEL 
...
void PSG_SetNoise(uint8_t type, uint8_t shift_rate) {
  // Construct payload and send latch byte
  uint8_t payload = 
    SN76489AN_LATCH_BYTE | (SN76489AN_NOISE_CHANNEL << 5) | SN76489AN_SHIFT_RATE_SEL 
    | ((type & 0x01) << 2) | (shift_rate & SN76489AN_LATCH_SHIFT_RATE_MASK);
  PSG_Write(payload);
  PSG_Wait();
}